<template>
    <div>
        <h2 class="page_title mb-4">북극곰 관리자 화면</h2>
        <table class="table mt-3">
            ...
            <tr v-for="(item, index) in apiData.slice().reverse()" :key="item.key"> <!-- key를 index로 사용 -->
                ...
                <td style="width: 75px; font-size:12px;">{{ item.size }}</td>
        ...
        <div class="paidTable">
            ...
            <dl style="margin-right: 20px;">
                <dt>입금 총계</dt>
                <dd>{{ (paidApplicantsCount * 30000).toLocaleString() }}원</dd>
            </dl>
...
export default {
    data(){
        return {
            apiUrl : "https://bonobono-e6ed4-default-rtdb.asia-southeast1.firebasedatabase.app/bear.json",
            exemptCountApiUrl : "https://bonobono-e6ed4-default-rtdb.asia-southeast1.firebasedatabase.app/bear_exemptCount.json",
            nonAttendeeCountApiUrl : "https://bonobono-e6ed4-default-rtdb.asia-southeast1.firebasedatabase.app/bear_nonAttendeeCount.json",
            apiData:[],
            result   :'',
            applicantCount: 0,
            exemptCount: 0,
    ...
    mounted(){
        ...
        const dataRef = ref(this.db, 'bear'); // cheering 경로에 대한 참조
