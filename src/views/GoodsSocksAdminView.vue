<template>
    <div>
        <h2 class="page_title mb-4">양말 구매 관리자 화면</h2>
        <table class="pay_table table mt-3" v-for="(item, index) in apiData.slice().reverse()" :key="item.key">
            <tbody>
                ...
                <tr :key="item.key + '-details'">
                    <td colspan="6" style="border:0px;">주문 : {{ item.order }}</td>
                </tr>
                <tr :key="item.key + '-pay'">
        ...
        <div class="paidTable">
            ...
            <dl style="margin-right: 20px;">
                <dt>입금 총계</dt>
                <dd>{{ (paidGoodsCapSum).toLocaleString() }}원</dd>
            </dl>
            <!--<dl style="margin-right: 20px;">-->
            <!--    <dt>실제차액</dt>-->
            <!--</dl>-->
...
export default {
    data(){
        return {
            apiUrl : "https://bonobono-e6ed4-default-rtdb.asia-southeast1.firebasedatabase.app/goodsSocks.json",
            apiData:[],
            result   :'',
    ...
    mounted(){
        ...
        const dataRef = ref(this.db, 'goodsSocks'); // cheering 경로에 대한 참조
    ...
    methods:{
        ...
        getData() {
            ...
            .then(response => {
                ...
                this.paidGoodsCapSum = this.apiData.reduce((sum, item) => {
                    return sum + (item.pay ? parseInt(item.pay, 10) : 0); // 입금한 금액만 합산
                }, 0);
                console.log('paidGoodsCapSum', this.paidGoodsCapSum);
