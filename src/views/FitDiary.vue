                ...
                <tr v-for="rank in 5" :key="'rank-' + rank">
                    ...
                    <td>
                        <div v-if="runRank[rank - 1]">
                            <div style="display:inline-block; font-weight: bold; border-bottom: 1px solid #aaa;" @click="goToEdit(runRank[rank - 1][0])">
                                {{ runRank[rank - 1][0] }}
                            </div>
                    ...
                    <td>
                        <div v-if="gymRank[rank - 1]">
                            <div style="display:inline-block; font-weight: bold; border-bottom: 1px solid #aaa;" @click="goToEdit(gymRank[rank - 1][0])">
                                {{ gymRank[rank - 1][0] }}
                            </div>
            ...
            <div class="fit-user-box" v-for="([userName, user], index) in allRecords" :key="userName">
                ...
                <div class="fit-progress-bar" v-if="(user.goals && user.goals['gym'] > 0) || getCount(user, 'gym') > 0">
                    <div class="fit-progress-fill fit-gym" :style="{ width: getPercent(user, 'gym') + '%' }">
                        헬스/클핏 {{ getPercent(user, 'gym') }}% ({{ getCount(user, 'gym') }}회)
                    </div>
