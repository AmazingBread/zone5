<template>
    <div>
        ...
        <div class="paidTable">
            ...
            <dl style="margin-right: 20px;">
                <dt>입금 총계</dt>
                <dd>{{ (paidApplicantsCount * 30000).toLocaleString() }}원 / {{ apiData.length* 30000 }}원</dd>
            </dl>
...
export default {
    data(){
        return {
            ...
            exemptCountPath: 'mt_exemptCount',
            nonAttendeeCountPath: 'mt_nonAttendeeCount',
            apiData:[],
            result   :'',
            applicantCount: 0,
            exemptCount: 0,
    ...
    mounted(){
        ...
        onValue(this.dataRef, (snapshot) => {
            ...
            this.apiData = Object.keys(getData)
                .filter(key => key !== 'mt_exemptCount') // exemptCount를 제외
                .map(key => ({
    ...
    methods:{
        ...
        togglePayment(key) {
            ...
            if (applicant) {
                const newPaidStatus = !applicant.paid;
                const applicantRef = ref(this.db, `${this.dbPath}/${key}`);

                update(applicantRef, { paid: newPaidStatus })
                .then(() => {
        ...
        fetchExemptCount() {
            const countRef = ref(this.db, this.exemptCountPath);
            get(countRef).then(snapshot => {
                const data = snapshot.val();
                this.exemptCount = data?.count || 0;
            }).catch(error => {
                console.error('면제자 수 가져오기 오류:', error);
            });
        },

        fetchNonAttendeeCount() {
            const countRef = ref(thsis.db, this.nonAttendeeCountPath);
            get(countRef)
            .then(response => {
        ...
        addExemptCountDatabase(count) {
            const countRef = ref(this.db, this.exemptCountPath);
            set(countRef, { count })
            .then(() => console.log('면제 인원 수 업데이트 완료:', count))
