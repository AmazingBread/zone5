<template>
    <div>
        <h2 class="page_title mb-4">팀주 경주 대회 6월 15일 일요일</h2>
        ...
        <p class="text-center text-bg-light" style="padding:10px;">📢📢📢 팀주 신청서 🏅🏅🏅🏅</p>
        <div class="mt-3">
        ...
        <div class="paidTable">
            ...
            <dl style="margin-right: 20px;">
                <dt>입금 총계</dt>
                <dd>{{ (paidApplicantsCount * 0).toLocaleString() }}원 / 45,000원 x{{ paidApplicantsCount }}명</dd>
            </dl>
        </div>
    </div>
</template>
...
export default {
    data(){
        return {
            apiUrl : "https://bonobono-e6ed4-default-rtdb.asia-southeast1.firebasedatabase.app/swimming_competition2.json",
            formData  :{
                checked      :'',
                name      :'',
                age      :'',
    ...
    mounted(){
        ...
        const dataRef = ref(this.db, 'swimming_competition2'); // cheering 경로에 대한 참조
        onValue(dataRef, (snapshot) => {
            const getData = snapshot.val() || {};
            this.apiData = Object.keys(getData)
                .filter(key => key !== 'swimming_competition2_exemptCount') // exemptCount를 제외
                .map(key => ({
